...
public class Sort1 extends Operator {
    ...
    public Sort1(Operator base, ArrayList<Attribute> as, int numbuff) {
        super(OpType.SORT);
        this.base = base;
    ...
    private void generateSortedRuns() {
        ...
        while (batch != null) {
            ...
            String fname = getSortedRunfname(0, totalnumrun);
            try {
    ...
    private int mergeSortedRuns(int numpass, int numrun) {
        if (numrun == 1) {
            ...
            try {
                String fname = getSortedRunfname(numpass - 1, numrun - 1);
    ...
    private void mergeSortedRunsRange(int start, int end, int numpass, int numrun) {
        ...
        String ofname = getSortedRunfname(numpass, numrun);
        ...
        for (int i = 0; i < end - start; i++) {
            String fname = getSortedRunfname(numpass - 1, start + i);
            try {
            ...
            try {
                ...
                try {
                    instream[i].close();
                    String dfname = getSortedRunfname(numpass - 1, start + i);
        ...
        while (!pq.isEmpty()) {
            ...
            if (!done[runid] && nexttupleid % batchsize == 0) {
                ...
                try {
                    ...
                    String dfname = getSortedRunfname(numpass - 1, start + runid);
                    File f = new File(dfname);
                    f.delete();
                } catch (ClassNotFoundException c) {
                    System.err.println("Sort: Error in deserialising temporary file ");
    ...
    public Batch next() {
        ...
        try {
            ...
            try {
                insorted.close();
                String dfname = getSortedRunfname(totalnumpass - 1, 0);
                File f = new File(dfname);
                f.delete();
            } catch (IOException io) {
                System.err.println("Sort: Error in reading temporary file");
